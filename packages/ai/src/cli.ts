...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @lc/ai <command> [provider]
...
Examples:
  npx @lc/ai login              # interactive provider selection
  npx @lc/ai login anthropic    # login to specific provider
  npx @lc/ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @lc/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @lc/ai --help' for usage`);
	process.exit(1);
}
