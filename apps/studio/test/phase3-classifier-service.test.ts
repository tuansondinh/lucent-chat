/**
 * Phase 3: ClassifierService on Host
 *
 * Tests:
 * - evaluateRules: returns 'deny' for deny rules before allow rules
 * - evaluateRules: returns 'allow' for allow rules when no deny match
 * - evaluateRules: glob wildcard matching (* and ?)
 * - evaluateRules: bash matches against 'command' arg
 * - evaluateRules: edit/write matches against 'file_path' arg
 * - evaluateRules: returns null when no rule matches
 * - evaluateRules: toolName scoping (rules for bash don't match edit)
 * - classifyToolCall: cache hit within 30s skips API call
 * - classifyToolCall: graceful degradation when no API key (approve = accept-on-edit behavior)
 * - classifyToolCall: paused pane returns approved=false with source='fallback'
 * - classifyToolCall: rate limiting queues excess calls beyond 5 concurrent per pane
 * - Block tracking: 3 consecutive blocks → paused
 * - Block tracking: 20 total blocks → paused
 * - Block tracking: consecutive resets on approval
 * - resume(): clears paused state and consecutive count
 * - getPaneState(): returns correct stats object
 */
