/**
 * Phase 3 — bridge.ts token URL cleanup tests.
 *
 * Verifies that after a deep-link bootstrap (URL contains ?server=&token=),
 * the bridge module strips those params from the URL bar via history.replaceState
 * so the token is not visible in address bar or browser history.
 */
