/**
 * Phase 3 unit tests — pane-store dirty state lifecycle (close confirmation flow).
 *
 * These tests cover the scenarios called out in PLAN-8.md Phase 3 acceptance
 * criteria:
 *   - set dirty
 *   - save clears dirty
 *   - external reload on clean tab
 *   - external reload blocked on dirty tab
 *   - close confirmation flow (hasDirtyTabs, discardDraft, closeFile)
 *
 * The tests validate the store API that the UI's close-guard dialog relies on.
 */

import { describe, it, expect, beforeEach } from 'vitest'
import { createPaneChatStore, deletePaneStore } from './pane-store'

describe('pane-store Phase 3 — dirty state lifecycle', () => {
  const PANE_ID = 'phase3-test-pane'
