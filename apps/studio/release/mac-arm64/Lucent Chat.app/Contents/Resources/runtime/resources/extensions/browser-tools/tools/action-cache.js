...
export function registerActionCacheTools(pi, deps) {
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            intent: Type.Optional(Type.String({ description: "Semantic intent key (for get/put). E.g., 'submit_form', 'close_dialog'." })),
            selector: Type.Optional(Type.String({ description: "CSS selector to cache (for put)." })),
            score: Type.Optional(Type.Number({ description: "Confidence score 0–1 for the cached selector (for put)." })),
        }),
